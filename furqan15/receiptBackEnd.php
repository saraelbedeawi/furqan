<?php
    ...
    else if( $_REQUEST["selectedButtonValue"] )
    {
        ...
        // "SELECT options.name ,options.type,options.html,options.id, donation_options.id as donationOptionId from options INNER JOIN donation_options ON options.id=donation_options.options_id where donation_options.donation_types_id=$value"
        ...
        $sql="SELECT options.name ,options_types.type,options.id, donation_options.id as donationOptionId from options INNER JOIN options_types on options.option_type_id=options_types.id INNER JOIN donation_options ON options.id=donation_options.options_id 
         where donation_options.donation_types_id=$value";
        ...
        if ($result->num_rows > 0) 
        {
            ...
            { 
               ...
               echo'<div id='.$row["donationOptionId"].' class="offset-md-4 col-md-3">';
                  ...
                  echo'<input id='.$row["id"].' type="'.$row["type"].'"></input>';
            ...
            <button onclick="saveReceipt()">save</button></div>
            </div>';
         }    
    }
	...
	function addReceipt($conn)
	{
        ...
        //insert recipt
        $sql="INSERT INTO `receipt`(`user_id`, `amount`, `donar_name`,
